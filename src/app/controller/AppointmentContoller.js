...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (provider_id === req.userId) {
      return res
        .status(401)
        .json({ message: 'You can not create appointments with yourself' });
    }
    ...
    if (checkAvailability) {
      return res
        .status(400)
        .json({ message: 'Appointment date is not avaiable' });
    }
  ...
  async delete(req, res) {
    ...
    if (appointment.user_id !== req.userId) {
      return res.status(401).json({
        message: 'You do not have permition to cancel this appointment',
      });
    }
